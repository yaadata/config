# How to run `git town status reset`
...
The status reset command deletes the persisted runstate. This is only needed if
the runstate is corrupted and causes Git Town to crash.
...
2. Inspect command-specific flags with `git town status reset --help`.
...
https://github.com/git-town/git-town/blob/main/website/src/commands/status-reset.md
